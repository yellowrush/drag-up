...
这是给小朋友用的极简网页：打开链接，输入名字和游戏想法，可以画图或拍照，提交后自动创建 GitHub Issue，并在网页里看到最近任务的链接和状态。
...
当前版本会在前端把图片压缩成小图，并随 Issue 正文一起发送。GitHub Issues API 没有网页拖拽上传那种附件接口；如果后续需要保存大图或原图，建议再接对象存储，或给 API token 增加 Contents 权限后把图片写入仓库目录再在 Issue 里引用。
